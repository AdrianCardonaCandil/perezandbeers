...
const Front = ({ logo, message, label }) => {
  return (
    <section className="flex items-center justify-center md:h-screen md:min-h-150">
      <div className="my-10 flex w-1/2 flex-col items-start gap-5 md:mt-50 md:w-2/3 fade-out">
        <h1
          className="fade-in text-5xl font-extrabold"
          style={{ fontFamily: "'Dancing Script'" }}
        >
          {logo}
        </h1>
        <p className="fade-in text-xs font-light">{message}</p>
        <img
          className="fade-in hidden md:block"
          src={divider}
          alt="section divider"
        ></img>
        <button className="fade-in p-1.5 text-sm text-[var(--primary-light)] inset-ring-2 inset-ring-[var(--primary-light)]">
          {label}
        </button>
      </div>
    </section>
